...
class Assets {
	...
	public static function exists(name:String):Bool {
		return FileSystem.exists(Paths.assetsPath(name));
	}

	public static function getDirs(dir:String):Array<String> {
		if (!FileSystem.isDirectory(Paths.assetsPath(dir))) {
			trace("None Found :C");
			return [];
		}
		return FileSystem.readDirectory(Paths.assetsPath(dir));
	}
		...
		try {
			...
			switch (type) {
				...
				case IMAGE:
					...
					return cache.get(path);
				case SPARROW:
					return FlxAtlasFrames.fromSparrow(load(IMAGE, FileSystem.absolutePath(path)), load(XML, FileSystem.absolutePath(path).replace(".png", ".xml")));
				case SOUND:
					if (!cache.exists(FileSystem.absolutePath(path)))
